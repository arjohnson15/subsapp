...
// Remove user from specific Plex server group
    async removePlexServerAccess(serverGroup) {
        ...
        const confirmMessage = `Remove ALL Plex access for ${userData.name}?\n\nThis will:\n- Cancel any pending invites on ALL servers\n- Remove them from ALL Plex servers\n- Clear ALL library access\n- Remove ALL Plex tags\n- Remove ALL Plex subscriptions\n\nThis action cannot be undone!`;
        ...
        try {
            ...
            // Remove ALL Plex access using user-specific removal endpoint
            const result = await API.call(`/users/${userId}/remove-plex-access`, {
                method: 'POST'
            });
            ...
            if (result.success) {
                const summary = result.summary;
                Utils.showNotification(
                    `Complete Plex removal successful! ` +
                    `${summary.invitesCancelled} invites cancelled, ` +
                    `${summary.usersRemoved} users removed, ` +
                    `${summary.removedTags.length} tags removed.`,
                    'success'
                ...
                // Clear ALL Plex-related checkboxes
                this.clearAllPlexCheckboxes();
                
                // Update status for both server groups
                ['plex1', 'plex2'].forEach(sg => {
                    const statusContainer = document.getElementById(`${sg}Status`);
                    if (statusContainer) {
    ...
    // Clear ALL Plex-related checkboxes (helper function)
    clearAllPlexCheckboxes() {
        // Clear all server group checkboxes
        ['plex1', 'plex2'].forEach(serverGroup => {
            ...
            // Uncheck the tag
            const tagNumber = serverGroup === 'plex1' ? '1' : '2';
            const tagCheckbox = document.querySelector(`input[name="tags"][value="Plex ${tagNumber}"]`);
            if (tagCheckbox) tagCheckbox.checked = false;
        });
        
        // Clear Plex email field
        ...
        console.log(`?? Cleared ALL Plex checkboxes`);
    },
