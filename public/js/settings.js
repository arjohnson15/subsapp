...
async function handleUpdatePlaylistNow() {
    try {
        ...
        updateBtn.innerHTML = `
            ...
            Updating...
            <small style="display: block; font-weight: normal;">This may take several minutes</small>
        `;
        ...
        if (result.success) {
            ...
            document.getElementById('lastAutoUpdaterRun').textContent = new Date().toLocaleString();
        ...
        updateBtn.innerHTML = 'Update Playlist Now';
    }
},

// Extract provider settings from selected playlist
async function extractProviderSettings(playlistId, playlistName) {
    try {
        if (!playlistId) return;
        
        console.log('🔍 Extracting provider settings from playlist:', playlistName);
        
        // Get the stored playlists data
        const response = await fetch('/api/iptv-editor/playlists');
        const result = await response.json();
        
        if (result.success && result.data && result.data.length > 0) {
            const selectedPlaylist = result.data.find(p => p.id === playlistId);
            
            if (selectedPlaylist && selectedPlaylist.patterns && selectedPlaylist.patterns.length > 0) {
                const pattern = selectedPlaylist.patterns[0];
                
                if (pattern.param1 && pattern.param2 && pattern.url) {
                    ...
                    // Update UI fields
                    document.getElementById('providerBaseUrl').value = providerSettings.provider_base_url;
                    document.getElementById('providerUsername').value = providerSettings.provider_username;
                    document.getElementById('providerPassword').value = providerSettings.provider_password;
                    ...
                    console.warn('⚠️ No valid provider patterns found for selected playlist');
                }
            }
            
        } else {
            console.warn('⚠️ No playlist data found');
        }
...
function initAutoUpdaterListeners() {
    ...
    const updateBtn = document.getElementById('updatePlaylistBtn');
    if (updateBtn) {
        updateBtn.addEventListener('click', handleUpdatePlaylistNow);
    }
    ...
    if (playlistSelect) {
        playlistSelect.addEventListener('change', (e) => {
            ...
            if (selectedOption && selectedOption.value) {
                extractProviderSettings(selectedOption.value, selectedOption.text);
            }
        });
    }
},
...
document.addEventListener('DOMContentLoaded', initAutoUpdaterListeners);
